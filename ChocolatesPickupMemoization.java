...
You are given a 2D matrix grid[][] of size n*m, where each cell grid[i][j] 
represents the number of chocolates available at position (i, j).
...
public class ChocolatesPickupMemoization {
    public int findMaxChocolate(int i, int j1, int j2, int grid[][], int dp[][][]) {
        int n = grid.length;
        int m = grid[0].length;
        if (i == n)
            return 0;
        ...
        for (int k = 0; k < 3; k++) {
            for (int l = 0; l < 3; l++) {
                maxi = Integer.max(maxi, findMaxChocolate(i + 1, j1 + dir[k], j2 + dir[l], grid, dp));
            }
        }
        maxi += grid[i][j1];
        if (j1 != j2)
    ...
    public int maxChocolate(int grid[][]) {
        ...
        return findMaxChocolate(0, 0, m - 1, grid, dp);
    }
}
