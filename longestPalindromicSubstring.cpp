/* Longest Palindrome in a String
    ...
    Input : s = “abc”
    Output : “a”
    Explanation : "a", "b" and "c" are longest palindromic substrings of same length.So, the first occurrence is returned.Constraints : 1 ≤ s.size() ≤ 103 s consist of only lowercase English letters.
    */
...
public:
    ...
    string longestPalindrome(string &s)
    {
        ...
        {
            string odds = expandAround(s, i, i);
            if (odds.length() > longest.length())
            {
                longest = odds;
            }
            string evens = expandAround(s, i, i + 1);
            if (evens.length() > longest.length())
            {
                longest = evens;
            }
        }
