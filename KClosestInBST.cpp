/* Putting this question not for logic, which is very simple, but for the c++ code. Templates ka asli use in DSA
   seekhne ko mila from this C++ code. */

/* K closest Values
Difficulty: MediumAccuracy: 82.24%Submissions: 4K+Points: 4
...
public:
    template <typename t>
    void inorder(Node *root, int target, int k, priority_queue<pair<int, int>, vector<pair<int, int>>, t> &pq)
    {
        if (root == nullptr)
            return;
        ...
        temp.second = abs(root->data - target);
        pq.push(temp);
        if (pq.size() == k + 1)
        {
            pq.pop();
